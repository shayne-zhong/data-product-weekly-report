...
test("task artifact upload stores metadata on the task and hides storage keys", async () => {
  ...
  const artifact = await service.upload({ state, departmentId: "data", taskId: "t1", actor, file: pdfFile, save: async () => {} });
...
test("task artifact reads are department scoped and deletion rolls back on save failure", async () => {
  ...
  await assert.rejects(() => service.readOriginal({ state, departmentId: "data", taskId: "t1", actor: { ...actor, departmentId: "other" } }), (error) => error.statusCode === 404);
  await assert.rejects(() => service.remove({ state, departmentId: "data", taskId: "t1", actor, save: async () => { throw new Error("save failed"); } }), /删除产物失败/);
