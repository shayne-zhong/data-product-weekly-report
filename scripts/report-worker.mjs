...
export const defaultWorkerRoot = join(process.env.LOCALAPPDATA || join(homedir(), ".local", "share"), "DepartmentWorkbench", "report-worker");
...
export async function loadBrowser() {
  const choices = [process.env.PLAYWRIGHT_MODULE_PATH, join(homedir(), ".cache", "codex-runtimes", "codex-primary-runtime", "dependencies", "node", "node_modules", "playwright", "index.mjs")].filter(Boolean);
...
export async function checkReportBrowser(html, { screenshotPath } = {}) {
  ...
  try {
      ...
      const issues = await page.evaluate(() => {
        ...
        for (const s of document.querySelectorAll(".slide")) {
          ...
          for (const e of s.querySelectorAll("h1,h2,h3,p,.summary,.source,.number,.note")) {
            ...
            if (e.scrollWidth > e.clientWidth + 2 || (e.closest(".body") && r.bottom > footer.top + 2)) problems.push(`${s.dataset.slideId}: ${e.tagName} 超出边界`);
          }
        }
    ...
    if (!(await page.locator(".slide").first().evaluate((s) => s.classList.contains("active")))) errors.push("Home 翻页失败");
    if (screenshotPath) await page.screenshot({ path: screenshotPath });
    await page.setViewportSize({ width: 390, height: 844 }); await inspect("窄屏");
    await page.setViewportSize({ width: 1920, height: 1080 });
    await page.emulateMedia({ media: "print" }); await inspect("打印");
    if (network.length) errors.push("报告试图请求外部资源");
  } finally { await browser.close(); }
  return { passed: !errors.length, slides, htmlHash: hash(html), errors: [...new Set(errors)].slice(0, 20), checkedAt: Date.now(), modes: ["desktop-all-slides", "mobile", "print", "navigation"] };
}
...
async function runCodex(job, dir, repair, config) {
  const windowsCli = join(process.env.APPDATA || join(homedir(), "AppData", "Roaming"), "npm", "node_modules", "@openai", "codex", "bin", "codex.js");
  const executable = process.platform === "win32" ? process.execPath : "codex";
  if (process.platform === "win32" && !(await exists(windowsCli))) throw new Error("本机未找到 Codex CLI，请先安装并登录");
  const args = [ ...(process.platform === "win32" ? [windowsCli] : []), "-c", 'service_tier="fast"', "-a", "never", "exec", "--skip-git-repo-check", "--sandbox", "workspace-write", "--json", "-C", dir, "-c", 'model_reasoning_effort="high"' ];
  ...
  return new Promise((resolveRun, reject) => {
    ...
    let stderr = "", output = "", bytes = 0, stopped = false;
    const stop = () => {
    ...
    child.stdout.on("data", (chunk) => { bytes += chunk.length; if (bytes > 8 * 1024 * 1024) stop(); else output += chunk.toString(); });
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0) return reject(new Error(`Codex 执行失败（${code}）：${stderr.replace(/(?:sk-|Bearer\s+)[\w-]+/g, "[已隐藏]").slice(-400)}`));
      let usage = null;
      for (const line of output.split("\n")) { try { const event = JSON.parse(line); if (event.usage) usage = event.usage; } catch { /* Non-JSON diagnostic. */ } }
      resolveRun({ usage });
    });
...
export function createWorkerClient(config) {
  ...
  if (!["http:", "https:"].includes(site.protocol) || site.username || site.password || site.search || site.hash) throw new Error("工作台网址无效");
  if (!config.token || !config.workerId) throw new Error("缺少本机连接配置");
  return async (action, body = {}) => {
    const response = await fetch(new URL(`/api/report-worker/${action}`, site), { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${config.token}` }, body: JSON.stringify(body), signal: AbortSignal.timeout(20_000), redirect: "error" });
    const result = await response.json();
    if (!response.ok) throw Object.assign(new Error(result.error || `网站返回 ${response.status}`), { statusCode: response.status });
    return result;
  };
}

export async function executeReportJob(job, { config, root, call, generate = runCodex, check = checkReportBrowser }) {
  const dir = join(root, config.workerId, job.id); await mkdir(dir, { recursive: true });
  const pendingPath = join(dir, "pending.json"), recordPath = join(dir, "run.json");
  let record = await exists(recordPath) ? JSON.parse(await readFile(recordPath, "utf8")) : { attempts: 0, lease: job.lease };
  if (record.lease !== job.lease) throw new Error("本地任务记录与执行归属不一致");
  let lost = false, beating = false;
  const heartbeat = setInterval(async () => {
    if (beating) return; beating = true;
    try { await call("heartbeat", { jobId: job.id, lease: job.lease }); }
    catch (e) { if ([401, 409].includes(e.statusCode)) lost = true; }
    finally { beating = false; }
  }, 20_000);
  try {
    if (await exists(pendingPath)) {
      const pending = JSON.parse(await readFile(pendingPath, "utf8"));
      await call("complete", pending); await saveJson(recordPath, { ...record, completed: true }); return;
    }
    ...
    if (record.generating && !(await exists(join(dir, "report.json")))) throw new Error("上次生成中断且未留下产物，请在后台重试");
    while (true) {
      ...
      } catch (e) { errors = [`报告 JSON 无效：${e.message}`]; }
      let browserCheck;
      if (!errors.length) {
        const html = renderReport(report); await writeFile(join(dir, "report.html"), html);
      ...
      if (!errors.length) { output = { jobId: job.id, lease: job.lease, report, browserCheck }; break; }
      record = { ...record, generating: false, errors }; await saveJson(recordPath, record);
    }
    ...
    if (!(await exists(pendingPath)) && ![401, 409].includes(error.statusCode)) {
      try { await call("fail", { jobId: job.id, lease: job.lease, error: error.message }); } catch { /* Retry the durable local record after reconnection. */ }
    }
    throw error;
...
async function acquireLocalLock(path) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try { const handle = await open(path, "wx", 0o600); await handle.writeFile(String(process.pid)); await handle.close(); return async () => unlink(path).catch(() => {}); }
    catch (e) {
      if (e.code !== "EEXIST") throw e;
      const pid = Number(await readFile(path, "utf8"));
      let existing = false;
      try { process.kill(pid, 0); existing = true; }
      catch (error) {
...
async function main() {
  const args = process.argv.slice(2), configArg = args.indexOf("--config");
  ...
  try {
    ...
    do {
      try {
        ...
        if (job) { console.log(`处理报告 ${job.id}`); await executeReportJob(job, { config, root, call }); console.log(`报告 ${job.id} 已回传`); }
        else if (args.includes("--once")) console.log("当前没有排队任务");
      } catch (error) { console.error(error.message); if (args.includes("--once")) throw error; }
