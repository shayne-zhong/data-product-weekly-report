...
export function createTaskArtifactService({
  store,
  convertOffice,
  makeKey = randomUUID,
  now = Date.now,
  ...
  async function upload({ state, departmentId, taskId, actor, file, save }) {
    ...
    const previewBuffer =
      previewKind === "office" ? await convertOffice({ buffer: file.buffer, extension }) : file.buffer;
  ...
  async function readArtifact({ state, departmentId, taskId, actor, preview }) {
    ...
    try {
      ...
      const officePreview =
        preview && task.artifact.previewMimeType === "application/pdf" && task.artifact.mimeType !== "application/pdf";
      return {
        buffer,
        filename: officePreview
          ? `${path.basename(task.artifact.originalName, path.extname(task.artifact.originalName))}.pdf`
          : task.artifact.originalName,
