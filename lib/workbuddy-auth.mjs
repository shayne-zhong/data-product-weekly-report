...
export function bindWecomUserId(accounts, username, wecomUserId) {
  ...
  if (text(account.wecomUserId) && text(account.wecomUserId) !== normalizedWecomUser) {
    throw new Error("Website account already bound");
  }
...
export function applyDirectoryMappings(state, mappings, { departmentId, batchId } = {}) {
  ...
  for (const mapping of Array.isArray(mappings) ? mappings : []) {
    const account = accounts.find(
      (row) =>
        row.departmentId === departmentId && normalizedUsername(row.username) === normalizedUsername(mapping?.username),
    );
    if (!account) {
      summary.skipped += 1;
      continue;
