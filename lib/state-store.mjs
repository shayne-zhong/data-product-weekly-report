...
function strongEtag(value) {
  return String(value || "").replace(/^W\//, "") || null;
}
    ...
    async load() {
      ...
      if (blob) {
        try {
          ...
          if (result?.stream) {
            vercelBlobEtag = strongEtag(result.blob?.etag || result.etag);
    ...
    async save(state, { baseState } = {}) {
      ...
      if (blob) {
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
          try {
            const saved = await blob.put(blobPath, JSON.stringify(persistedState), {
              access: "private",
              allowOverwrite: true,
              contentType: "application/json; charset=utf-8",
              ...(vercelBlobEtag ? { ifMatch: vercelBlobEtag } : {}),
            });
            vercelBlobEtag = strongEtag(saved?.etag || saved?.blob?.etag);
            return persistedState;
          } catch (error) {
            const conflict = error?.name === "BlobPreconditionFailedError";
            if (!conflict || !baseState || attempt === 2) throw error;
            const latest = await blob.get(blobPath, { access: "private", useCache: false });
            if (!latest?.stream) throw error;
            vercelBlobEtag = strongEtag(latest.blob?.etag || latest.etag);
            const latestState = JSON.parse(await new Response(latest.stream).text());
            persistedState = applyStateChanges(latestState, baseState, state);
          }
        }
      }
