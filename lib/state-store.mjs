...
export function defaultLocalStatePath(pid = process.pid) {
  return join(tmpdir(), `data-product-weekly-report-state-v1-${pid}.json`);
}
    ...
    async load() {
      ...
      if (blob) {
        try {
          ...
          if (env.VERCEL) throw new Error(`Cloud state load failed: ${error.message || error}`);
        }
        return null;
      }
