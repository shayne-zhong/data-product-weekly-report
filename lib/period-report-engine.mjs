...
export function validateReport(r) {
  ...
  const str = (v, p, max) => {
    if (typeof v !== "string" || !v.trim() || v.length > max) fail(p, `需为1–${max}字文本`);
  };
  const list = (v, p, min, max) => {
    if (!Array.isArray(v) || v.length < min || v.length > max) fail(p, `需为${min}–${max}项数组`);
  };
  const ids = (values, p) => {
    const set = new Set();
    array(values).forEach((v, i) => {
      str(v?.id, `${p}[${i}].id`, 60);
      if (set.has(v?.id)) fail(p, "ID重复");
      set.add(v?.id);
    });
    return set;
  };
  ...
  str(r.title, "title", 60);
  str(r.organization, "organization", 40);
  ...
  list(r.domains, "domains", 1, 12);
  list(r.sources, "sources", 0, 500);
  list(r.slides, "slides", 1, 40);
  const domains = ids(r.domains, "domains"),
    sources = ids(r.sources, "sources");
  ids(r.slides, "slides");
  array(r.domains).forEach((d, i) => {
    str(d?.label, `domains[${i}].label`, 20);
    if (!/^#[0-9a-f]{6}$/i.test(d?.color)) fail(`domains[${i}].color`, "需为#RRGGBB");
  });
  array(r.sources).forEach((s, i) => str(s?.title, `sources[${i}].title`, 200));
  const refs = (v, p, required) => {
    list(v, p, required ? 1 : 0, 10);
    array(v).forEach((id) => {
      if (!sources.has(id)) fail(p, `未知来源 ${id}`);
    });
  };
  array(r.slides).forEach((s, i) => {
    ...
    const hasMetrics = ["metrics", "combined"].includes(s.kind),
      hasCards = ["cards", "combined"].includes(s.kind);
    ...
    array(s.metrics).forEach((m, j) => {
      ...
      str(m.label, `${q}.label`, 20);
      str(m.definition, `${q}.definition`, 48);
      ...
      if (m.progress !== undefined) {
        try {
          completion(m.progress?.actual, m.progress?.target);
        } catch (e) {
          fail(`${q}.progress`, e.message);
        }
    ...
    array(s.cards).forEach((c, j) => {
      ...
      str(c.title, `${q}.title`, 24);
      str(c.body, `${q}.body`, 110);
      refs(c.sourceIds, `${q}.sourceIds`, true);
...
export function renderReport(r) {
  ...
  const card = (c) =>
    `<article class="card">${c.value ? `<div class="number">${esc(c.value)}</div>` : ""}<h3>${esc(c.title)}</h3><p>${esc(c.body)}</p>${source(c.sourceIds)}</article>`;
