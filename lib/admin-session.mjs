...
export async function issueAdminToken({ username, now = Date.now(), ttlMs = defaultTtlMs, env = process.env }) {
  const normalizedUsername = String(username || "").trim().toLowerCase();
  const expiresAt = now + ttlMs;
  const payload = encode(JSON.stringify({ username: normalizedUsername, issuedAt: now, expiresAt }));
