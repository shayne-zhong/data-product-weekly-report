...
import { OFFICE_EXTENSIONS } from "./goal-artifact-core.mjs";
...
export async function convertOfficeToPdf({
  ...
  try {
    ...
    await run(executable, ["--headless", "--convert-to", "pdf", "--outdir", temporaryDirectory, sourcePath], { timeout: 30_000 });
    ...
    if (error?.killed || error?.code === "ETIMEDOUT" || error?.signal === "SIGTERM") throw httpError(504, "Office 预览转换超时");
