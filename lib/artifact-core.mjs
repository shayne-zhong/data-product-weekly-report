...
const signatures = {
  ...
  zip: (buffer) =>
    hasPrefix(buffer, [0x50, 0x4b]) && [0x03, 0x05, 0x07].includes(buffer[2]) && [0x04, 0x06, 0x08].includes(buffer[3]),
...
export function mergeGoalRows(_existingRows = [], incomingRows = [], makeId) {
...
export function publicArtifact(artifact) {
  return artifact
    ? {
        ...
        updatedBy: artifact.updatedBy || null,
      }
    : null;
}
...
export function publicGoalRows(rows = []) {
  return rows.map((row) => {
    const publicRow = { ...row };
    delete publicRow.artifact;
    delete publicRow.canManageArtifact;
    return publicRow;
  });
}
