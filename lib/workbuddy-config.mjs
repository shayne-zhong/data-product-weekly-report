...
export function publicWorkbuddyConfig(state, { env = process.env } = {}) {
  ...
  return {
    ...
    oauthResolverUrl: text(stored.oauthResolverUrl) || envValue(env, "WORKBUDDY_OAUTH_RESOLVER_URL"),
    corpId: text(stored.corpId) || envValue(env, "WECOM_OAUTH_CORP_ID"),
    openApiToken: tokenProjection(stored.openApiToken, envValue(env, "WORKBUDDY_OPEN_API_TOKEN")),
    oauthResolverToken: tokenProjection(stored.oauthResolverToken, envValue(env, "WORKBUDDY_OAUTH_RESOLVER_TOKEN")),
  };
}
...
export function validateWorkbuddyConfigPatch(body = {}) {
  ...
  if (body.clear_oauth_resolver_token === true && Object.hasOwn(body, "oauth_resolver_token")) {
