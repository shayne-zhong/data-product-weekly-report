...
export function adminScope(actor, departments, requestedDepartmentId = "") {
  ...
  if (actor?.role === "leader" && departments.some((department) => department.id === actor.departmentId)) {
    return {
...
export function validateAdminSettingsTransition(current, next) {
  // Keep current in the transition contract for comparisons added by future safeguards.
  ...
  if (invalidModuleLeader) {
    reject(400, "模块负责人至少负责一个模块，且该模块必须属于本部门");
  }
}
