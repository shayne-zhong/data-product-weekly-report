...
const resultValues = new Set(["success", "failed", "skipped", "retrying", "failure", "retry", "conflict", "rejected"]);
...
function compareEvents(left, right) {
  return Number(right.occurredAt) - Number(left.occurredAt) || String(right.id).localeCompare(String(left.id));
}
...
export function pruneSyncEvents(state, { now = Date.now() } = {}) {
  ...
  target.syncEventIds = Object.fromEntries(
    target.syncEvents.filter((event) => text(event.externalEventId)).map((event) => [event.externalEventId, event.id]),
  );
  return target.syncEvents;
}

export function appendSyncEvent(state, input, { now = Date.now(), idFactory = () => `sync_${randomUUID()}` } = {}) {
  const target = bucket(state);
...
export function querySyncEvents(state, options = {}) {
  ...
  const filtered = target.syncEvents.filter(
    (event) =>
  ...
  return {
    events,
    nextBefore: filtered.length > events.length && events.length ? encodeCursor(events.at(-1)) : "",
  };
}
...
export function summarizeSyncEvents(state, { now = Date.now() } = {}) {
  ...
  for (const event of bucket(state).syncEvents) {
    const result = summaryBucket[event.result] || event.result;
    if (event.occurredAt >= threshold && Object.hasOwn(summary, result)) {
      summary[result] += 1;
    }
  }
  return summary;
