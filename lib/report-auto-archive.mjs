...
function archiveExecution(state) {
  if (!state.reportArchiveExecution || typeof state.reportArchiveExecution !== "object")
    state.reportArchiveExecution = {};
...
export function failReportArchiveExecution(state, { now = Date.now(), error } = {}) {
  ...
  Object.assign(execution, {
    status: "failed",
    finishedAt: now,
    error: String(error?.message || error || "执行失败").slice(0, 200),
  });
  return execution;
}
...
function scheduledTimestamp(report, schedule) {
  const match = String(report.data?.endDate || report.endDate || "")
    .replaceAll("/", "-")
