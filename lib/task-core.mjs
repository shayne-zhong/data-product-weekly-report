...
export function normalizeTaskGoalLinks(overrides = {}) {
  ...
  if (!goalLinks.length && overrides.goalId) {
    goalLinks.push({
      ...
      contribution: Number.isFinite(Number(overrides.goalContribution))
        ? Math.max(0, Number(overrides.goalContribution))
        : 0,
...
export function buildEmptyTask(overrides = {}) {
  ...
  return {
    ...
    progress: Number.isFinite(Number(overrides.progress))
      ? Math.max(0, Math.min(100, Math.round(Number(overrides.progress))))
      : 0,
    ...
    goalContribution: Number.isFinite(Number(overrides.goalContribution))
      ? Math.max(0, Number(overrides.goalContribution))
      : 0,
...
export function applyTaskStatus(task, nextStatus, { blocker = "", now = Date.now() } = {}) {
  ...
  const goalLinks =
    Array.isArray(task.goalLinks) && task.goalLinks.length
      ? task.goalLinks
      : task.goalId
        ? [{ goalId: task.goalId, contribution: task.goalContribution }]
        : [];
...
export function rolloverTasks(
  tasks,
  { targetWeekId, sourceWeekId, existingTargetTasks = [], now = Date.now(), idFactory = null },
) {
  const carriedSourceIds = new Set(existingTargetTasks.map((task) => task.sourceTaskId).filter(Boolean));
  return tasks
    .filter((task) => shouldRolloverTask(task) && !carriedSourceIds.has(task.id))
    .map((task, index) => ({
      ...buildEmptyTask({
        ...task,
...
function normalizeModules(modules = defaultReportModules) {
  const rows = (Array.isArray(modules) ? modules : []).map((item) => String(item || "").trim()).filter(Boolean);
