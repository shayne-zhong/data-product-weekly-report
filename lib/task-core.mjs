...
export function buildEmptyTask(overrides = {}) {
  ...
  return {
    ...
    module: overrides.module || "数据治理与经营分析",
...
export function shouldRolloverTask(task) {
  return task.status !== "已完成" || Boolean(task.carryToNextWeek) || task.status === "阻塞" || Boolean(task.blocker);
}

export function rolloverTasks(tasks, { targetWeekId, sourceWeekId, existingTargetTasks = [], now = Date.now() }) {
  ...
  return tasks.filter((task) => shouldRolloverTask(task) && !carriedSourceIds.has(task.id)).map((task, index) => ({
    ...buildEmptyTask({
      ...task,
      id: `task_${now.toString(36)}_${index}`,
      weekId: targetWeekId,
      status: task.status,
...
function pushGrouped(target, moduleName, line) {
  const modules = ["AI+X项目", "AI应用项目", "数据治理与经营分析", "财经共享"];
  const key = modules.includes(moduleName) ? moduleName : "数据治理与经营分析";
  if (!target[key]) target[key] = [];
  target[key].push(line);
}

export function summarizeTasksForReport(tasks) {
  ...
  tasks.filter((task) => task.includeInReport !== false).forEach((task) => {
    if (task.status === "已完成") {
      ...
      pushGrouped(summary.progress, task.module, progress ? `${task.title}：${progress}` : task.title);
      return;
    }

    if (task.status === "阻塞" || task.blocker) {
      pushGrouped(summary.risks, task.module, task.blocker ? `${task.title}：${task.blocker}` : task.title);
    }

    if (task.status !== "已完成" || task.carryToNextWeek) {
      pushGrouped(summary.next, task.module, task.title);
    }
  });
