...
> 后续运维扩展以 `2026-08-29-workbuddy-sync-admin-design.md` 为准。该扩展允许后台加密配置覆盖环境变量，并增加管理员映射维护和同步日志；本文件定义的任务查询、完成回写和 OAuth 业务契约保持不变。
...
1. 两个任务开放 API 使用同一个部门级 Bearer Token；初始部署从服务端环境变量读取，启用后续运维后台时可由加密的后台配置覆盖。
...
`WORKBUDDY_DIRECTORY_MAPPINGS_JSON` 与批次 ID 用于内网部署时执行一次幂等通讯录初始化；后续批次必须使用新的批次 ID。任务开放 API Token 与 OAuth 身份解析 Token 必须分离。任何密钥都不得以明文进入前端持久状态、业务状态、普通错误响应或日志；后续运维后台只允许加密持久化并返回掩码。
